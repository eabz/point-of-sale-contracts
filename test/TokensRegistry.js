...
describe('TokensRegistry', () => {
    ...
    it('should revert add token with an empty address', async () => {
        await expect(this.registry.addToken("0x0000000000000000000000000000000000000000", DAI_WETH))
            .to.revertedWith('TokensRegistry: missing token')
    });

    it('should revert add token with an empty pair', async () => {
        await expect(this.registry.addToken(DAI, "0x0000000000000000000000000000000000000000"))
    ...
    it('should revert to add an already added token', async () => {
        await expect(this.registry.addToken(DAI, DAI_WETH))
            .to.revertedWith("TokensRegistry: the token is already supported")
    });

    it('should return true for a supported token', async () => {
        await expect(await this.registry.isSupported(DAI))
            .to.eq(true)
    });

    it('should return false for a not supported token', async () => {
        await expect(await this.registry.isSupported(WETH))
            .to.eq(false)
    });

    it('should return false for a not paused token', async () => {
        await expect(await this.registry.isPaused(DAI))
            .to.eq(false)
    });
    ...
    it('should fail when trying to pause a non supported token', async () => {
        await expect(this.registry.pauseToken(WETH))
            .to.revertedWith("TokenRegistry: the token is not supported")
    });
    ...
    it('should fail when trying to unpause an non paused token', async () => {
        await expect(this.registry.resumeToken(DAI))
            .to.revertedWith("TokenRegistry: the token is not paused")
    });

    it('should fail when trying to unpause an non supported token', async () => {
        await expect(this.registry.resumeToken(WETH))
            .to.revertedWith("TokenRegistry: the token is not supported")
    });
